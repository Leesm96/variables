...
export interface TasteDeclineRow {
  issue: string;
  total_count: number;
...
const persistFeedback = (records: FeedbackRecord[]): void => {
  ensureStorage();
...
const validatePayload = (payload: Partial<FeedbackPayload>): string | null => {
...
const buildRecord = (payload: FeedbackPayload): FeedbackRecord => {
  return {
    id: randomUUID(),
    issue: payload.issue,
...
const groupTasteDecline = (records: FeedbackRecord[]): TasteDeclineRow[] => {
  const summary: Record<
    string,
    { total: number; bad: number; reasons: Record<string, number> }
  > = {};
  ...
  req
    ...
    .on('end', () => {
      try {
        ...
        const record = buildRecord(payload as FeedbackPayload);
